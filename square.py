...
def sdf_square(xy, center, half_side):
        ...
        Công thức toán học:
        φ(x) = -max(|x - cx| - a, |y - cy| - a, 0)  cho điểm ngoài hình vuông
        φ(x) = -max(|x - cx| - a, |y - cy| - a)     cho điểm trong hình vuông
        
        Trong đó:
        - (cx, cy) là tâm hình vuông
        - a là nửa độ dài cạnh (half_side)
        - Cạnh hình vuông = 2*a
        
        Giá trị trả về:
        - φ(x) > 0: điểm nằm BÊN TRONG hình vuông (vật cản)
        - φ(x) = 0: điểm nằm TRÊN BIÊN hình vuông
        - φ(x) < 0: điểm nằm BÊN NGOÀI hình vuông (tự do)
        
        Args:
            xy: tensor (N, 2) chứa tọa độ các điểm cần tính
            center: tuple (cx, cy) - tâm hình vuông
            half_side: float - nửa độ dài cạnh
        
        Returns:
            tensor (N,) chứa giá trị φ(x) cho mỗi điểm
        """
        cx, cy = center
        x, y = xy[:, 0], xy[:, 1]
        
        # Bước 1: Tính khoảng cách có dấu theo từng chiều
        # dx = |x - cx| - a  (dương nếu x nằm ngoài hình vuông theo chiều x)
        # dy = |y - cy| - a  (dương nếu y nằm ngoài hình vuông theo chiều y)
        dx = torch.abs(x - cx) - half_side
        dy = torch.abs(y - cy) - half_side
        
        # Bước 2: Xác định điểm nằm trong hay ngoài hình vuông
        inside = (dx <= 0) & (dy <= 0)  # điểm nằm trong hình vuông
        outside = ~inside                # điểm nằm ngoài hình vuông
        
        # Bước 3: Tính φ(x) cho điểm nằm ngoài
        # Với điểm ngoài: φ = - (khoảng cách Euclidean đến hình vuông)
        # Dấu âm để φ < 0 cho bên ngoài
        phi_outside = -torch.sqrt(dx.clamp(min=0)**2 + dy.clamp(min=0)**2)
        
        # Bước 4: Tính φ(x) cho điểm nằm trong
        # Với điểm trong: φ = -max(dx, dy) (giá trị dương)
        # Vì dx, dy ≤ 0 nên -max(dx, dy) ≥ 0
...
class VisibilityNet2D(nn.Module):
    def __init__(self, square_center, half_side):
        super().__init__()
        # Lưu thông tin vật cản
        self.register_buffer("square_center", square_center.to(DEVICE))
        self.register_buffer("half_side", half_side.to(DEVICE))
        self.register_buffer("x_star", X_STAR)  # Điểm quan sát x*
        
        # Tính φ(x*) - giá trị tại điểm quan sát (luôn âm vì x* nằm trong vật cản)
        with torch.no_grad():
            self.register_buffer("varphi_xs", sdf_square(X_STAR.unsqueeze(0), square_center, half_side)[0])
    ...
    def _varphi(self, xy): 
        ...
        return sdf_square(xy, self.square_center, self.half_side)
...
def main():
    ...
    square_center = torch.tensor([2.0, 2.0], device=DEVICE)
    half_side = torch.tensor(0.5, device=DEVICE)
    # Khởi tạo mạng với tham số ban đầu θ₀ (dòng 1-2)
    model = VisibilityNet2D(square_center, half_side).to(DEVICE)
